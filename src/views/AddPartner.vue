                    ...
                    <v-row>
                        <v-form ref="form" v-model="form">
                            <v-col class="d-inline-flex"> 
                                <v-text-field v-model="email" :rules="[rules.email]" filled label="email of your partner" type="email"> </v-text-field>
                                <v-btn @click="searchUserByEmail" :disabled="!form" large color="primary" class="ml-2 mt-2">Search</v-btn>
                            </v-col>
                        </v-form>
                    </v-row>
                        ...
                        <v-col class="d-inline-flex">
                            <v-card>
                                <v-card-text>
                                    ...
                                    <div class="text-primary">To add this User, click the button below</div>
                                </v-card-text>
                                <v-card-actions>
...
export default {
    ...
    methods: {
        async searchUserByEmail() {
            ...
            this.message = "";
            this.foundUser=null
            const snapshot = await db.collection("users").where("email", "==", this.email.toLowerCase()).get();
            if (!snapshot.email) {
