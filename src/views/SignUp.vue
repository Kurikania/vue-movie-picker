<template>
    <v-card class="mx-auto" style='max-width: 500px'>
        ...
        <v-form ref="form" v-model="form" class="pa-4 mt-6">
            ...
            <v-text-field v-model="email" :rules="[rules.email]" filled label="email" type="email">
            </v-text-field>
            <v-text-field v-model="password" :rules="[rules.password, rules.length(6)]" filled counter="6" label="password" type="password" style="min-height: 96px">
            </v-text-field>
        </v-form>
        ...
        <v-card-actions>
            ...
            <v-spacer></v-spacer>
            <v-btn :disabled="!form" color="primary" :loading="isLoasding" depressed @click="signup">Sign Up</v-btn>
        </v-card-actions>
    </v-card>
</template>
   ...
   methods: {
       async signup() {
           ...
           try {
               ...
               await db.collection('users').doc(authRes.user.uid).set({name: this.name, email: this.email})
